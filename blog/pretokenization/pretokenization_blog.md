...
Most people treat the tokenizer as a fixed black box. Text goes in, token IDs come out, and whatever happens in between is someone else's problem. But there is a step *before* the tokens which is a plain regular expression that quietly shapes how well a model handles your input. Once you can read it, you can exploit it, and in many cases you can get real quality gains **without training anything**.
...
**Prompt endings (branch 6, trailing whitespace).** A trailing space becomes its own token, and the model then has to predict a non-space-prefixed continuation which is statistically rare, since most word tokens carry a leading space. Strip it:
